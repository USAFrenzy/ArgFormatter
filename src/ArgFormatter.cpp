...
// TODO: Rework this file to be header-only 
// NOTE: Need to add back the wstring type in the UTF-Utils project, forgot it was used over here
...
namespace formatter::arg_formatter {
	...
	static constexpr utf_helper::se_wstring LocalizedFormat(const unsigned char& ch) {
		utf_helper::se_wstring tmp;
		switch( ch ) {
				case 'c': [[fallthrough]];
				case 'x': [[fallthrough]];
				case 'C': [[fallthrough]];
	...
	void ArgFormatter::FormatSubseconds(int precision) {
		...
		if( !specValues.localize ) {
				buffer[ valueSize++ ] = '.';
				std::to_chars(begin + valueSize, begin + formatter_ARG_BUFFER_SIZE, subSeconds);
		}
		valueSize += precision;
	}
