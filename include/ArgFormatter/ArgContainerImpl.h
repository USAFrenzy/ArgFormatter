...
#include "ArgContainer.h"
	...
	constexpr std::array<formatter::VType, MAX_ARG_COUNT>& ArgContainer::ArgStorage() {
		return argContainer;
	}
	...
	constexpr unsigned int& ArgContainer::uint_state(size_t index) {
		AF_ASSERT(index <= MAX_ARG_INDEX, "Error Retrieving Stored Value - Index Is Out Of Bounds");
		AF_ASSERT(std::holds_alternative<unsigned int>(argContainer[index]), "Error Retrieving unsigned int: Variant At Index Provided Doesn't Contain This Type.");
	...
	constexpr formatter::CustomValue& formatter::msg_details::ArgContainer::custom_state(size_t index) {
		AF_ASSERT(index <= MAX_ARG_INDEX, "Error Retrieving Stored Value - Index Is Out Of Bounds");
		// clang-format off
		AF_ASSERT(std::holds_alternative<formatter::CustomValue>(argContainer[index]),
